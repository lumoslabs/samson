...
class KubernetesDashboardController < ApplicationController
  ...
  def release(releases, release_id, role_id, deploy_group_id)
    releases[release_id] ||= {
      ...
      target_replicas: target_replicas(deploy_group_id, release_id, role_id),
      live_replicas: 0
    }
  end
  ...
  def target_replicas(deploy_group_id, release_id, role_id)
    Kubernetes::ReleaseDoc.find_by(kubernetes_release_id: release_id, kubernetes_role_id: role_id,
      deploy_group_id: deploy_group_id).replica_target
  end
